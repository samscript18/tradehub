		...
		<div className="px-4 md:px-6 py-5 space-y-6">
			<section className="dashboard-panel rounded-2xl p-5 md:p-7">
				<div className="flex flex-col gap-2">
					<p className="text-xs tracking-[0.18em] uppercase text-slate-400">Merchant Workspace</p>
					<h1 className="text-2xl md:text-3xl font-bold text-white">Order History</h1>
					<p className="text-sm text-slate-300">Manage incoming orders and monitor fulfillment progress.</p>
				</div>
			</section>
				...
				{isLoading ? (
					...
					<motion.div className="grid grid-cols-1 gap-5" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.5 }} transition={{ staggerChildren: 0.2, delayChildren: 0.3 }}>
						{data?.data?.map((order) => {
							...
							return (
								<Card key={order._id} className="dashboard-panel dashboard-glow-hover rounded-2xl cursor-pointer" onClick={() => router.push(`/merchant/orders/${order._id}`)}>
									<CardContent className="p-5 md:p-6">
										<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
											<div className="">
												<div className="flex gap-3 flex-wrap items-center">
													<h3 className="text-xs max-md:text-center text-slate-400 mb-2 tracking-[0.18em]">ORDER ID</h3>
													<h3 className="text-xs max-md:text-center text-slate-200 mb-2 font-medium">{order._id.toUpperCase()}</h3>
												</div>
													...
													<div className="flex-1 max-md:text-center">
														...
														<div className="text-sm text-gray-400 space-y-1">
															{order?.products.map((item, idx) => {
																...
																return (
																	<div key={idx} className="flex items-center gap-4 text-slate-300/90">
																		<Image
