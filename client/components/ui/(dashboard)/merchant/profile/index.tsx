...
import { getBanks, getMerchant, getWalletBalance, initiateWithdraw, updateMerchant, validateAccountInfo } from "@/lib/services/merchant.service";
...
const MerchantProfilePage = () => {
	...
	return (
		<div className="min-h-screen text-white px-4 md:px-6 py-5 space-y-6">
			<div className="relative h-64 w-full overflow-hidden rounded-2xl dashboard-panel">
			...
			<div className="w-full dashboard-panel rounded-2xl border border-slate-700/70 mb-2">
				<div className="w-full flex space-x-4 md:space-x-8 px-6 pt-4">
					{tabs.map((tab) => (
						<button
							key={tab.id}
			...
			{activeTab === "basic-info" && (
				<form onSubmit={handleSubmit(submit)} className="dashboard-panel rounded-2xl p-6 space-y-8">
							...
							<div className="space-y-2">
								<MultiSelectField
									...
									onSearch={(search: string) => {
										return storeCategories.filter((category) => category.label.toLowerCase().includes(search.toLowerCase()));
									}}
									initialOptions={
					...
					<div className="max-md:w-full flex justify-center items-center gap-4 pt-6 pb-8">
						<Button type="submit" className="max-md:w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg">
			...
			{activeTab === "documents" && (
				<div className="dashboard-panel rounded-2xl p-6 space-y-8">
					<h2 className="text-xl font-medium">Business Documents</h2>

					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div className="dashboard-input rounded-xl border border-slate-700/70 overflow-hidden">
							...
							<div className="border-t border-slate-700/70 p-4">
								<Button variant="outline" className="w-full dashboard-input border-slate-700 text-slate-300 hover:bg-slate-800">
			...
			{activeTab === "banking" && (
				<div className="dashboard-panel rounded-2xl p-6 space-y-8">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<>
							...
							{isDropDownOpen && searchQuery && (
								<div className="w-full max-h-[200px] dashboard-input border border-slate-700/70 rounded-xl shadow-md p-4 overflow-y-scroll">
									{searchingBanks || (!banks && debouncedSearch) ? (
										...
										banks?.map((bank) => (
											<div
												key={bank.code}
												className="py-2 px-3 mb-2 bg-slate-900/80 shadow-md rounded-md cursor-pointer"
												onClick={() => {
							...
							<div className="mt-2">
								...
								{isAccountNumberValid && (
									<div className="mt-2">
										...
										<div className="text-[.85rem] pt-1">{resolvingAccountInfo ? <Loader size={25} text="Fetching account name..." /> : accountInfo?.account_name}</div>
									</div>
								)}
							</div>
						)}
					...
					<div className="flex justify-center items-center">
						<Button
							disabled={!accountNumber || !amount || !accountInfo?.account_name || Number(deFormatValue(amount)) > (wallet?.balance ?? 0)}
							onClick={async () => {
