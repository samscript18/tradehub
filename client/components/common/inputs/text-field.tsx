...
import { AnimatePresence, HTMLMotionProps, Variant, motion } from 'framer-motion';
import React, { HTMLAttributes, InputHTMLAttributes, ReactElement, useState } from 'react';
...
export default function TextField({
	...
	return (
		<div {...props} className={`flex gap-1 flex-col w-full relative ${props.className}`}>
			<AnimatePresence>
				{label && (
					<motion.label
						{...LabelProps}
			...
			{helperText && (
				<p {...helperTextProps} className={`text-red-500 text-[.8rem] ${helperTextProps?.className}`}>
					{helperText}
				</p>
			)}
		</div>
