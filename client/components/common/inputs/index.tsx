...
const SelectField: FC<Props> = ({ data, value, onClear, onSelect, loading, onSearch, className, disabled, placeholder, showDropDownSuffix = true }) => {
	...
	return (
		<div className={cn("relative w-[200px]", className)}>
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div variants={dropdownVariant} initial="hidden" animate="visible" exit="hidden" className="absolute bg-white shadow-md w-full z-[20]">
...
export const Option: FC<OptionsProps> = ({ option, index, selectedId, onSelect }) => {
	...
	return (
		<div ref={ref} onClick={() => onSelect?.(option)} className={cn("p-2 !text-[.85rem] cursor-pointer", focused && "bg-mainLight text-white", !focused && "hover:bg-mainExtraLight")}>
