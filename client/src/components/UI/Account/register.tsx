...
import { setupPassword, signUp, verifyEmail } from '@/lib/services/auth.service';
...
const Register = () => {
	...
	const { mutate: setupPasswordMutate, isPending: settingUpPassword } = useMutation({
							...
							<div className="space-y-2 col-span-2">
								...
								<div
									...
									<div className="cursor-none flex items-center gap-2 text-gray-400">
										<Image src={'/svgs/nigeria-flag.svg'} width={24} height={24} alt="flag" />
				...
				return (
					<motion.div
						...
						<p className="text-sm text-gray-500">
							Didn&apos;t get the code? <span className="text-secondary">Resend code</span>
						</p>
					</motion.div>
				);
			case 3:
						...
						<div className="space-y-8">
							...
							<div className="bg-gray-100 rounded-full h-1 max-w-2xl relative overflow-hidden">
								<motion.div
									...
									className="absolute left-0 top-0 rounded-full bg-accent h-full"></motion.div>
							</div>
						</div>
							...
							<AnimatePresence mode="wait" initial={false}>
								{step === 1 ? (
									<motion.div key={step} className="space-y-2">
										<h1 className="text-4xl font-bold">Create a Stockly account</h1>
										<p className="text-sm text-gray-500">Start your stress free business management journey here.</p>
									</motion.div>
								) : step === 2 ? (
									<motion.div key={step} className="space-y-2">
										<h1 className="text-4xl font-bold">Confirm email address</h1>
										<p className="text-sm text-gray-500">
											We&apos;ve sent a six digit code to <span className="text-gray-700">{email}</span>
										</p>
									</motion.div>
								) : (
									<motion.div key={step} className="space-y-2">
							...
							{step === 1 && (
								<>
									...
									<p className="text-center text-gray-400 text-sm">
										...
										<Link href="/login" className="text-secondary border-b pb-[1px] border-secondary">
											Login
										</Link>
									</p>
									...
									<div className="grid grid-cols-2 gap-2">
										<button className="px-4 py-2 w-full border flex items-center justify-center gap-2 border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-900 dark:hover:text-slate-300 transition duration-300">
											<Image className="size-5" src="/svgs/google-color.svg" alt="google logo" width={24} height={24} />
											<span>Continue with Google</span>
										</button>

										<button className="px-4 py-2 w-full border flex items-center justify-center gap-2 border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-900 dark:hover:text-slate-300 transition duration-300">
											<Image className="size-5" src="/svgs/x-logo.svg" alt="x logo" width={24} height={24} />
											<span>Continue with X</span>
										</button>
									</div>

									<p className="text-gray-500 text-sm max-w-lg mx-auto text-center">
										By continuing, you agree to the <span className="text-secondary">General Terms of Use</span>,{' '}
										<span className="text-secondary">Merchant Terms of Use</span> &{' '}
										<span className="text-secondary">General Privacy Policy</span> of Stockly
									</p>
								</>
							)}
						</div>
				...
				<div className="w-screen h-screen flex items-center justify-center">
					<div className="space-y-5 text-center">
						<div className="flex items-center justify-center">
							<Image alt="confetti" src="/images/items/confetti.gif" width={100} height={100} />
						</div>
