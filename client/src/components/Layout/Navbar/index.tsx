		...
		<nav className="fixed top-0 left-0 z-[100] w-full bg-white">
			{showBanner && (
				<div className="bg-primary text-white text-sm text-center py-2">
					<p>
						Join TradeHub today and grow your local business online — Connect, sell, and deliver with ease. Sign
						up now!
					</p>
				</div>
			)}
				...
				<ul className="flex space-x-6">
					{navLinks.map((link, idx) => (
						<li key={idx} className="relative group">
							<Link href={link.href!} className="text-text hover:text-primary font-medium">
								{link.title}
							</Link>
						</li>
					))}
