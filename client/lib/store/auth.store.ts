			...
			async fetchUser() {
				try {
					...
					if (user.role === "customer") {
						...
						set({
							user: { ...user, storeName, storeLogo, storeDescription, storeCategory, isVerified, addresses: merchantAddresses, notificationsDisabled: user.notificationsDisabled } as User &
								Merchant,
						});
					}
				} catch {
