...
export const useCart = create<CartStore>((set, get) => ({
	...
	loadItems: () => {
		...
		const savedItems = storage?.getItem("cart-items");
		if (savedItems) {
	...
	addItem: (product: Product, quantity: number, variant?: ProductVariant) => {
		...
		getSafeLocalStorage()?.setItem("cart-items", JSON.stringify(newItems));
		set({ items: newItems });
	},
	...
	updateQuantity: (id: string, newQuantity: number) => {
		...
		const newItems = newQuantity === 0 ? get().items.filter((item) => item._id !== id) : get().items.map((item) => (item._id === id ? { ...item, quantity: newQuantity } : item));
